...
public class ShoulderRedpandaFeatureRenderer<T extends PlayerEntity> extends FeatureRenderer<T, PlayerEntityModel<T>> {
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T playerEntity, float f, float g, float h, float j, float k, float l) {
        this.renderShoulderRedPanda(matrixStack, i, playerEntity, f, g, k, l, true);
        this.renderShoulderRedPanda(matrixStack, i, playerEntity, f, g, k, l, false);
    }

    private void renderShoulderRedPanda(MatrixStack matrices, int light, T player, float limbAngle, float limbDistance, float headYaw, float headPitch, boolean leftShoulder) {
        ...
        id.filter((type) -> type == ModEntities.REDPANDA);
        id.ifPresent((type) -> {
            matrices.push();
            matrices.translate(leftShoulder ? 0.4F : -0.4F, player.isInSneakingPose() ? -1.3F : -1.5F, 0.0F);
            this.model.poseOnShoulder(matrices, null, light, OverlayTexture.DEFAULT_UV, limbAngle, limbDistance, headYaw, headPitch, player.age);
            matrices.pop();
        });
    }
