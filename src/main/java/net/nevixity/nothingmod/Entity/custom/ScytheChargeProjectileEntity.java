...
public class ScytheChargeProjectileEntity extends PersistentProjectileEntity {
    private static final TrackedData<Boolean> HIT =
            DataTracker.registerData(ScytheChargeProjectileEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public void tick() {
        ...
        for(int i = 1; i < 5; ++i) {
            this.getWorld().addParticle(ModParticles.SCYTHE_CHARGE_FLAME_PARTICLE, d0-(d5*1), d1-(d6*2), d2-(d7*1),
                    -d5, -d6 - 0.1D, -d7);
        }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (hurt) {
            if(hitEntity instanceof LivingEntity livingHitEntity) {
                livingHitEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.WITHER, 150, 0), owner);
                livingHitEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.DARKNESS, 150, 0), owner);
            }
        }
